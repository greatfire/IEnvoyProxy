package IEnvoyProxy
/*
	This tests connecting to an Envoy proxy with ECH using HTTP/2 and HTTP/3
*/
import (
	"context"
	"crypto/sha256"
...
//
func (e *EchProxy) getDigest(input string) (string) {
	temp := input + e.Salt
...
// a cache param
//
func (e *EchProxy) getTestRequest(ctx context.Context, u string) (*http.Request, error) {
	...
	if err != nil {
		log.Printf("Error parsing TargetUrl: %s, %s", e.TestTarget, err)
		return nil, err
	}
...
func (e *EchProxy) doTestHttps(ctx context.Context, useHttp3 bool, resultChan chan EnvoyResonse) (bool, error) {
	...
	log.Printf("Testing URL: %s", u)

	testUrl, err := url.Parse(u)
	if err != nil {
		log.Printf("Error parsing our own URL %s", err)
		return false, err
	}
	...
	if err != nil {
		if !errors.Is(err, context.Canceled) {
			log.Printf("Error requesting URL via HTTP", err)
		}
		return false, err
	}
...
//
func (e *EchProxy) testHttps() (string) {
	...
	case <-time.After(30 * time.Second):
		cancel()
		log.Printf("Timeout testing HTTPS")
		return ""
	}
}
